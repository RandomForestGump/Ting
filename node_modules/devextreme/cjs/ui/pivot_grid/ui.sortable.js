...
function getItemsOffset($elements, isVertical, $itemsContainer) {
    ...
    for (var i = 0; i < $elements.length; i += $item.length) {
        ...
        if ($item.is(":visible")) {
            ...
            if (isVertical) {
                offset.posVertical = ($item.last().offset().top + $item.offset().top + (0, _size.getOuterHeight)($item.last(), true)) / 2
            } else {
                offset.posVertical = (0, _size.getOuterHeight)($item.last(), true) + $item.last().offset().top
            }
            result.push(offset)
        }
    }
